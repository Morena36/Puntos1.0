...
#                )
#    if ARGS.headless and platform.system() == "Linux":
#        chrome_browser_obj.set_window_size(1920, 1080)
#    return chrome_browser_obj
...
def completeMSNShoppingGame(browser: WebDriver) -> bool:
    ...
    def signIn() -> None:
        ...
        while True:
            ...
            try:
                ...
                prYellow("[MSN GAME] Failed to locate login button !")
            ...
            try:
                browser.execute_script("""return document.querySelector("#root > div > div > fluent-design-system-provider > div > div:nth-child(4) > div > shopping-page-base")
                                                                        .shadowRoot.querySelector("div > div.shopping-page-header > common-header")
                                                                        .shadowRoot.querySelector("msn-verticals-header")
                                                                        .shadowRoot.querySelector("fluent-design-system-provider > div > div.top-container > div.user-pref-container > ms-rewards")
                                                                        .shadowRoot.querySelector("#msRewards > span > div.reward-points.reward-balance-contrast")
                                                                        .textContent""")
                break
            except Exception as exc:
                pass
    ...
    def getGamingCard() -> WebElement:
        ...
        for element in msn_shopping_game_pane:
            if element.get_attribute("gamestate"):
                return element
        return None

    def moveCards() -> None:
        """move up the cards"""
        browser.execute_script("""var msnShoppingGamePane = document.querySelector("shopping-page-base")
        ...
        time.sleep(calculateSleep(2))

    def closeProductCard() -> bool:
        """close product pop"""
        try:
            browser.execute_script("""document.querySelector(
                                                    "#root > div > div > fluent-design-system-provider > div > div:nth-child(1) > div:nth-child(1) > shopping-modal"
                                                ).shadowRoot.querySelector(
                                                    "#modalActionClose").shadowRoot
                                                .querySelector("button").click()""")
            return True
        except Exception:
            return False

    def getGamingCardStatus() -> str:
        """get gaming card status"""
        ...
        while True:
            try:
                ...
                time.sleep(calculateSleep(2))
                if gaming_card.get_attribute("gamestate") is not None:
                    return gaming_card.get_attribute("gamestate")
            except Exception:
                pass
    ...
    def closeAllTabs() -> None:
        """close all open tabs except one"""
        browser.switch_to.window(browser.window_handles[0])
        driver_len = len(browser.window_handles)
        if driver_len > 1:
    ...
    while True:
        try:
            ...
            gaming_card = getGamingCard()
            time.sleep(calculateSleep(2))
            if gaming_card.get_attribute("gamestate") == 'active':
                for i in range(10):
                    try:
                        if closeProductCard():
                            break
                        print(f'[MSN GAME] Answering {i+1}/10', end="\r")
                        clickCorrectAnswer()
                        clickPlayAgain()
...
def cleanLogs():
    ...
    del LOGS[CURRENT_ACCOUNT]["MSN shopping game"]
